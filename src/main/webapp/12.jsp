...
<body>
   ...
   ResultSet rs1 = DbConnect.getStatement().executeQuery("select c_id from HALLTICKET where HALLTICKET_NO = "+tid+""); 
  Long cid1=null;
   if (rs1.next()){
      cid1 = rs1.getLong(1);
    ...
    System.out.println("select * from QUESTION where SLNO=1 and C_ID="+cid1+"");
  	ResultSet rs =DbConnect.getStatement().executeQuery("select * from QUESTION where SLNO=1 and C_ID="+cid1+""); 
						...
						<h1>
							...
							<form action="exam">
								<input type="hidden" name="hdnHallTicket"> <input
									type="hidden" name="c_id" value="<%=cid1%>"> <input
